...
You can download the files separately as well
	a. GrCh37 - http://hgdownload.cse.ucsc.edu/goldenpath/hg38/bigZips/hg38.2bit
	b. GrCh38 - http://hgdownload.cse.ucsc.edu/goldenpath/hg19/bigZips/hg19.2bit
...
Once the script is run, it will ask for input to select certain parameters:
	a. **Genomic Build** - Enter 'A' for GrCh37 OR 'B' for GrCh38 (this genomic build will be used for mapping in the VCF)
	b. **Use Case** - The script either consumes a TSV file with headers *Gene name* and *genomic HGVS* with the data or it can take a single gene and genomic HGVS and create a VCF file. (Enter '1' for file option OR '2' for single gene and genomic HGVS option).
